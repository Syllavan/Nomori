...
namespace MudBun
{
  ...
  public abstract class MudRendererBase : MonoBehaviour
  {
    ...
    public struct Const
    {
      ...
      public static int NumRayMarchSteps;
    ...
    */
    ...
    private static NativeArray<int> IndirectDrawArgsInitData
    {
      get
      {
        ...
        if (s_indirectDrawArgsInitData.IsCreated)
        {
    ...
    private bool ShouldRenderBoxProxy
    {
      get
      {
    ...
    private bool m_prevShouldRenderBoxProxy = false;
    ...
    public bool UseRayMarchedRenderMaterial = false;
    ...
    (
      ...
      bool weldVertices = false // only respected if uvGeneration == UVGenerationMode.None
    )
    {
      Transform [] aBone;
    ...
    )
    {
      ...
      if (RenderModeCategory == RenderModeCategoryEnum.Splats)
      {
        RenderMode = RenderModeEnum.FlatMesh;
      }
    ...
    (
      ...
      bool weldVertices // only respected if uvGeneration == UVGenerationMode.None
    )
    {
      ...
      if (meshType == GeneratedMeshType.Standard)
      {
        ...
        if (uvGeneration == UVGenerationMode.None && weldVertices)
        {
          MeshUtil.Weld(mesh);
        }
      }
    ...
    */
    ...
    {
      //CompatibilityManager.TryCheckCompatibility();
    ...
    {
      //CompatibilityManager.EndCompatibilityCheck();
    ...
    private bool UpdateNeedsCompute()
    {
      ...
      // check need for rendering box proxy
      if (m_needsCompute)
      {
        bool shouldRenderBoxProxyChanged = (ShouldRenderBoxProxy != m_prevShouldRenderBoxProxy);
        m_prevShouldRenderBoxProxy = ShouldRenderBoxProxy;
        if (shouldRenderBoxProxyChanged)
          m_needsCompute = true;
      }
    ...
    public static void ValidateShaderConstantId()
    {
      ...
      Const.NumRayMarchSteps = Shader.PropertyToID("numRayMarchSteps");
    ...
    public void RescanBrushersImmediate()
    {
      ...
      int brushSetHash = ComputeBrushSetHash();
      if (brushSetHash != m_brushSetHash)
      {
        m_needsCompute = true;
        m_brushSetHash = brushSetHash;
      }
    }

    private int ComputeBrushSetHash()
    {
      int hash = 0;
    ...
    private void WriteResources()
    {
      ...
      float effectiveSurfaceShift = SurfaceShift;
    ...
    private void BindRenderResources()
    {
      ...
      if (MaterialNeedsSdfProperties(m_materialUsed))
      {
        ...
        m_materialProps.SetInt(Const.AabbRoot, m_aabbTree.Root);
        ...
        {
          //m_materialProps.SetFloat(Const.NumRayMarchSteps, NumRayMarchSteps);
          //m_materialProps.SetFloat(Const.RayMarchStepSize, RayMarchDistance / Mathf.Max(1, NumRayMarchSteps));
    ...
    private void Compute()
    {
      ...
      switch (MeshingMode)
      {
        case MeshingModeEnum.MarchingCubes:
          ...
          {
            case RenderModeEnum.FlatMesh:
            case RenderModeEnum.Decal: // TODO: just draw brush bounds
              if (Enable2dMode)
