...
float sdf_brush_apply(float res, float groupRes, SdfBrushMaterial groupMat, inout float3 p, SdfBrush b, inout SdfBrushMaterial oMat, float halfNodeDiag = -1.0f)
{
...
float sdf_masked_brushes(float3 p, int iBrushMask, out SdfBrushMaterial mat, float halfNodeDiag = -1.0f)
{
  int iStack = -1;
  ...
  FOR_EACH_BRUSH(iBrushMask, 
    ...
    res = sdf_brush_apply(res, groupRes, groupMat, groupP, aBrush[iBrush], mat, halfNodeDiag);
    if (res == kCull)
